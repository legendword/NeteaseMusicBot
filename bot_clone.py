...
async def leave(ctx):
    ...
    sq['songs'] = []
    if sq['voice_client'].is_connected():
...
@bot.command(name='sid')
async def play_id(ctx):
    sq = get_queue(ctx.guild.id, ctx.channel, ctx.voice_client)
    if sq['is_connected'] and sq['is_playing']:
        await ctx.send(sq['songs'][sq['pos']]['id'])
...
async def show_lyrics(ctx):
    sq = get_queue(ctx.guild.id, ctx.channel, ctx.voice_client)
    if sq['is_connected'] and sq['is_playing']:
        async with ctx.channel.typing():
            if sq['songs'][sq['pos']].get('name') == None:
                sq['songs'][sq['pos']] = fetch_song_info(sq['songs'][sq['pos']]['id'])
            lrc = fetch_lyrics(sq['songs'][sq['pos']]['id'])
            await ctx.send(embed=discord.Embed(title=(sq['songs'][sq['pos']]['name']+' - '+sq['songs'][sq['pos']]['artists']), description=lrc, type='rich', color=discord.Color.red()).set_author(name='Lyrics'))
        ...
        async with ctx.channel.typing():
            ...
            if smode == 'song':
                ...
                if len(sq['songs']) == 1:
                    ...
                    await ctx.send(embed=discord.Embed(title=('Queued: '+str(spos)+'. '+song_info['name']+' - '+song_info['artists']), description=('Currently Playing: '+str(sq['pos']+1)+'. **'+sq['songs'][sq['pos']]['name']+'** - '+sq['songs'][sq['pos']]['artists']+(' ***[VIP]***' if sq['songs'][sq['pos']]['vip']==True else '')), type='rich', color=discord.Color.red()))
            elif smode == 'playlist':
...
#        print(datetime.datetime.now().strftime('%y-%m-%d %H:%M:%S')+' Playback Finished '+sq['songs'][sq['pos']]['name']+' - '+sq['songs'][sq['pos']]['artists']+' (#'+str(sq['songs'][sq['pos']]['id'])+') ['+str(sq['pos'])+'/'+str(len(sq['songs']+']')))
    ...
    if ct - sq['song_start'] < 2.0:
        ...
        if sinf.get('name') == None:
            sinf = fetch_song_info(sinf['id'])
...
def fetch_lyrics(sid):
    http = urllib3.PoolManager()
    rs = http.request('GET', 'https://api.imjad.cn/cloudmusic/?type=lyric&id='+str(sid))
    sd = json.loads(rs.data.decode('utf8'))
    if sd['code'] == 200:
        return re.sub('(\[[a-zA-Z\w\d\:\.]+\])', '', sd['lrc']['lyric']) #remove timestamps
    else:
        return '[This song does not have any lyrics.]'
